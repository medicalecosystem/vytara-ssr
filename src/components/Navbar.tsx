...
import ThemeSelector from './FloatingThemeButton';
...
export default function Navbar() {
  ...
  return (
    <>
      <header
        className={`sticky top-0 z-40 w-full border-b border-teal-900/20 text-white md:hidden ${
          isOnboarding ? 'pointer-events-none opacity-70' : ''
        }`}
        style={{ backgroundColor: 'var(--theme-navbar)' }}
      >
        <div className="flex items-center justify-between px-4 py-3">
          ...
          >
            ...
            <div>
              <p className={`text-xs uppercase tracking-[0.25em] ${isDefaultTheme ? 'text-white-200/70' : 'text-white'}`}>G-1</p>
              <p className={`text-sm font-semibold leading-tight ${isDefaultTheme ? 'text-slate-900' : 'text-white'}`}>Patient Hub</p>
            </div>
          </button>
          <button
            onClick={async () => {
              ...
              <button
                ...
                className={`flex min-w-[8rem] flex-1 items-center justify-center gap-2 rounded-full px-4 py-2 text-xs font-medium transition ${
                  isActive
                    ...
                    : `${isDefaultTheme ? 'text-teal-100/80 hover:bg-white/10 hover:text-white' : 'text-white hover:bg-white/10 hover:text-white'}`
                }`}
              >
      ...
      <aside
        className={`sticky top-0 hidden h-screen shrink-0 border-r border-teal-900/20 text-white transition-[width] duration-200 md:block ${
          effectiveCollapsed ? 'w-20' : 'w-64'
        } ${isOnboarding ? 'pointer-events-none opacity-70' : ''}`}
        style={{ background: 'var(--theme-navbar)' }}
      >
          ...
          <div className="flex items-center justify-between px-1">
            ...
            >
              <div className="w-11 h-11 bg-white rounded-full flex items-center justify-center shadow-md p-2">
                <div className="w-full h-full rounded-full" style={{ backgroundColor: 'var(--theme-background)' }}></div>
              </div>
              {!effectiveCollapsed && (
                <div>
                  <p className="text-sm uppercase tracking-[0.25em] text-white-200/70">
                    Vytara
                  </p>
            ...
            className={`rounded-lg p-2 transition ${isDefaultTheme ? 'text-teal-100/70 hover:bg-white/10 hover:text-white' : 'text-white hover:bg-white/10 hover:text-white'}`}
              >
                {effectiveCollapsed ? (
        ...
        <nav className="mt-4 flex-1 space-y-2">
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  style={isActive ? { backgroundColor: 'var(--theme-sidebar-active)' } : {}}
          ...
          <button
            ...
            className={`mt-auto flex items-center gap-2 rounded-lg px-3 py-2 text-sm transition ${isDefaultTheme ? 'text-red-200/90 hover:bg-white/10 hover:text-red-100' : 'text-red-200/90 hover:bg-white/10 hover:text-red-100'}`}
          >
